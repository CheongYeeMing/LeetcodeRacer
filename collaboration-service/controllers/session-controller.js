...
const { difficultyOptions, categoriesOptions } = require('./data');
...
const handleConnection = async (ws, req) => {
  ...
  ws.on('message', (message) => {
    ...
    if (sessionUsers[sessionId] && sessionUsers[sessionId].includes(userId)) {
      ws.send(
        JSON.stringify({ allowed: true, usersInfo: sessionUsers[sessionId] })
      );
      if (
        activeSessions[sessionId].second !== ws.userId &&
        !activeSessions[sessionId].first
      ) {
        ...
        activeSessions[sessionId].first !== ws.userId &&
        !activeSessions[sessionId].second
      ) {
        activeSessions[sessionId].second = ws.userId;
        ws.send(JSON.stringify(randomQuestions[sessionId]));
  ...
  if (activeSessions[sessionId]) {
    if (activeSessions[sessionId].disconnectTimer !== undefined) {
      console.log(
        `Clearing disconnectTimer for sessionId ${sessionId}: ${activeSessions[sessionId].disconnectTimer}`
      );
...
const handleMessage = (message, ws, sessionId) => {
  ...
  if (type === 'language') {
    ...
    session.listeners.forEach((listenerWs) => {
      if (
        listenerWs.userId === otherUserId &&
        listenerWs.readyState === WebSocket.OPEN
      ) {
        listenerWs.send(
          JSON.stringify({ type: 'language', language: language })
        );
      }
    });
  }
  ...
  if (type === 'REQUEST_END_SESSION') {
    if (confirmEnd) {
      ...
      console.log(
        `userId: ${userId}, otherUser: ${otherUser}, otherUserId: ${otherUserId}`
      );
    }
  }
...
const handleClose = (ws, sessionId, confirmEnd) => {
    ...
    if (activeSessions[sessionId]) {
      if (activeSessions[sessionId].disconnectTimer === undefined) {
        activeSessions[sessionId].disconnectTimer = setTimeout(() => {
          if (
            activeSessions[sessionId] &&
            Array.isArray(activeSessions[sessionId].listeners)
          ) {
            activeSessions[sessionId].listeners.forEach((listenerWs) => {
              if (listenerWs.readyState === WebSocket.OPEN) {
                listenerWs.send(
      ...
      console.log(
        `Set disconnectTimer for sessionId ${sessionId}: ${activeSessions[sessionId].disconnectTimer}`
      );
    }
  }
  ...
  if (activeSessions[sessionId]) {
    if (
      activeSessions[sessionId].listeners.length === 0 &&
      activeSessions[sessionId].first === null &&
      activeSessions[sessionId].second === null
    ) {
      delete sessionUsers[sessionId];
  ...
  try {
    ...
    while (true) {
      ...
      const response = await axios.get(
        `http://${base_url}/questions/randomQuestion`,
        {
          data: {
            difficulty:
              questionComplexity === 'Any' ? complexity : questionComplexity,
      ...
      if (Object.keys(randomQuestion).length !== 0) {
