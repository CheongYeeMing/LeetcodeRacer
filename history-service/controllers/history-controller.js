...
const addHistory = async (req, res) => {
  ...
  if (!userId || !questionId || !sessionId || raceOutcome == undefined) {
    return res.status(422).json({
      message: `Invalid input, please enter a valid user id ${userId}, question id${questionId}, sessionId, and result`,
    });
  }
  ...
  try {
    ...
    if (existingRecord) {
      ...
      const outcomeForExistingRecord =
        existingRecord.score == score
          ? 0
          : existingRecord.score > score
          ? 1
          : 2;
      const outcomeForNewRecord =
        existingRecord.score == score
          ? 0
          : existingRecord.score > score
          ? 2
      ...
      await History.updateOne(
        { sessionId },
        { $set: { raceOutcome: outcomeForExistingRecord } }
      );
      console.log('test2');
      const historyEntry = new History({
        ...
        raceOutcome: outcomeForNewRecord,
        score: score,
  ...
  const userPromises = rankings.map(async (user) => {
    try {
      ...
      const res = await fetch(
        `http://${url}/users/getUser?userId=${user._id}`,
        {
          method: 'GET',
          headers: {
...
const getLeaders = async (req, res) => {
  ...
  try {
    const weekUsersRankings = await getUserNames(
      await History.aggregate([pastWeek, groups, addWinRate, sort, limit])
    );

    const monthUsersRankings = await getUserNames(
      await History.aggregate([pastMonth, groups, addWinRate, sort, limit])
    );

    const dayUsersRankings = await getUserNames(
      await History.aggregate([pastDay, groups, addWinRate, sort, limit])
    );
